...
if [[ $first_snapshot == "Y" ]]; then
	sudo zfs send -vw zroot/data/home@${time}-${hostname} | ssh truenas "zfs receive -F Vault/backups/${hostname}"
	notify-send "Backup Complete"
elif [[ $first_snapshot == "N" ]]; then
	sudo zfs send -vw -I ${old_snapshot} zroot/data/home@${time}-${hostname} | ssh truenas "zfs receive -F Vault/backups/${hostname}"
	notify-send "Backup Complete"
else
